...
template< class T> struct Vector : public std::vector<T> {
    ...
    {
        if (i < 0 || this->size() <= i) throw std::range_error(i);
...
struct Color {
    ...
    enum class Transparency { invisible = 0, visible=255 };

    Color(Color_type cc) :c(Fl_Color(cc)), v((char)Transparency::visible) { }
    Color(Color_type cc, Transparency vv) :c(Fl_Color((char)cc)), v((char)vv) { }
    Color(int cc) :c(Fl_Color((char)cc)), v((char)Transparency::visible) { }
    Color(Transparency vv) :c(Fl_Color()), v((char)vv) { }    // default color

    int as_int() const { return c; }

    char visibility() const { return v; } 
    void set_visibility(Transparency vv) { v=(char)vv; }
private:
...
struct Line_style {
    ...
    Line_style(Line_style_type ss) :s((int)ss), w(0) { }
    Line_style(Line_style_type lst, int ww) :s((int)lst), w(ww) { }
...
struct Arc : Ellipse {
    Arc(Point p, int ww, int hh, int minAng, int maxAng)	// center, min, and max distance from center
        :Ellipse{p, ww, hh}, minAngle(minAng), maxAngle(maxAng){}
...
struct Suffix {
    enum class Encoding { none, jpg, gif  };
};
...
//------------------------------------------------------------------------------
//Chapter 13 EX2, as far as I'll go with this one...
//struct Arrow : Shape {
...
struct Box : Shape {
    Box(Point centr, int w, int h, int cornerSize = 10);    //default 10
    void draw_lines() const;
    void set_color(Color c);
...
struct LabeledBox : Box {
    LabeledBox(Point centr, int w, int h, std::string labl, int cornerSize = 10);    //default 10
    void draw_lines() const;
