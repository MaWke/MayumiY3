...
//------------------------------------------------------------------------------
    ...
    struct Checkbox : Button {
        Checkbox(Point xy, int w, int h, const std::string& label, Callback cb, std::unique_ptr<bool>& boolRef)
            :Button(xy, w, h, label, cb), enabled(boolRef), visualBox(Point(xy.x+w/2, xy.y+h/2), w/2-5, h/2-5, label) {}

        void attach(Window&);
        void set_fill_color(Color::Color_type ct);

        void changeEnabled();   //returns current state
        bool isEnabled() const { return *enabled; }
        

    private:
        Graph_lib::LabeledBox visualBox;
